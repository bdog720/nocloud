...
class NoiseProtocol {
  ...
  Future<List<Uint8List>> _hkdf(
    ...
    for (int i = 1; i <= count; i++) {
      ...
      lastBlock = Uint8List.fromList(
        (await hmac.calculateMac(input, secretKey: SecretKey(prk))).bytes,
      );
  ...
  Future<Uint8List> createHandshake1() async {
    ...
    if (pubKey is! SimplePublicKey) {
      throw StateError('Expected SimplePublicKey');
    }
    ...
    debugPrint(
      'Noise after Msg1: ck=${hex(_ck)}, h=${hex(_h)}, payload=${hex(payload)}',
    );
