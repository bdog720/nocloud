...
class EspHomeConnectionService {
  ...
  Future<void> connect() async {
    if (kIsWeb) {
      throw UnsupportedError(
        'TCP Sockets are not supported on Flutter Web. Please use an Android or iOS device.',
      );
    }
    ...
    try {
      ...
      if (noisePsk != null) {
        final handler = NoiseProtocolHandler(
  ...
  void _onPayload(Uint8List payload) {
    ...
    try {
      ...
      if (_protocol is NoiseProtocolHandler) {
        if (payload.length < 4) {
          debugPrint(
            'EspHomeService: Payload too short for Noise framing: ${payload.length}',
          );
          return;
        }
        ...
        debugPrint(
          'EspHomeService: Noise Message Type: $typeId, Internal Length: $internalLen',
        );
      ...
      if (msg != null) {
        ...
        debugPrint(
          'EspHomeService: Failed to decode message for typeId: $typeId',
        );
      }
    } catch (e) {
  ...
  int _getMessageId(GeneratedMessage msg) {
    ...
    throw UnimplementedError(
      'Message ID not mapped for type ${msg.runtimeType}',
    );
  }
  ...
  GeneratedMessage? _decodeMessage(int typeId, List<int> payload) {
    switch (typeId) {
      case 2:
        return HelloResponse.fromBuffer(payload);
      case 6:
        return DisconnectResponse.fromBuffer(payload);
      case 8:
        return PingResponse.fromBuffer(payload);
      case 10:
        return DeviceInfoResponse.fromBuffer(payload);
      case 19:
        return ListEntitiesDoneResponse.fromBuffer(payload);
      case 46:
        return ListEntitiesClimateResponse.fromBuffer(payload);
      case 47:
        return ClimateStateResponse.fromBuffer(payload);
      default:
        return null;
    }
  }
