...
class NoiseProtocolHandler implements ProtocolHandler {
  ...
  Future<void> startHandshake() async {
    ...
    debugPrint(
      'Starting Noise handshake with PSK length: ${_pskBase64.length} (base64)',
    );
  ...
  Future<void> _processBuffer() async {
    debugPrint(
      'NoiseProtocolHandler: Starting _processBuffer, buffer size: ${_buffer.length}',
    );
    _processing = true;
    try {
      while (_buffer.isNotEmpty) {
        ...
        if (_buffer[0] != 0x01) {
          debugPrint(
            'NoiseProtocolHandler: Skipping non-indicator byte: ${_buffer[0]}',
          );
          _buffer.removeAt(0);
          continue;
        }
        if (_buffer.length < 3) {
          debugPrint(
            'NoiseProtocolHandler: Waiting for header (buffer size: ${_buffer.length})',
          );
          return;
        }

        final length = NoiseFraming.getLength(
          Uint8List.fromList(_buffer.sublist(0, 3)),
        );
        ...
        if (_buffer.length < 3 + length) {
          debugPrint(
            'NoiseProtocolHandler: Waiting for full frame (buffer size: ${_buffer.length}, need: ${3 + length})',
          );
          return;
        }
        ...
        if (_state == NoiseState.hello) {
          ...
          try {
            ...
            debugPrint(
              'Ready: Decrypted ${decrypted.length} bytes: ${_noise.hex(decrypted)}',
            );
  ...
  void _handleHelloResponse(Uint8List payload) {
    ...
    if (payload.isEmpty || payload[0] != 0x01) {
      ...
      _readyCompleter.completeError(
        Exception(
          'Invalid Server Hello protocol: ${payload.isNotEmpty ? payload[0] : "empty"}',
        ),
      );
      return;
    }
  ...
  Future<void> _handleHandshakeMessage(Uint8List msg) async {
    try {
      ...
      if (msg.isEmpty || msg[0] != 0x00) {
        throw Exception(
          'Invalid handshake message prefix: ${msg.isNotEmpty ? msg[0] : "empty"}',
        );
      }
